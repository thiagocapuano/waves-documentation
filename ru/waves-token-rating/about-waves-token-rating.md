...
В момент голосования записывается значение текущего баланса _**B **WCT. В течении 24 часов проверяются все **расходные **операции WCT по данному адресу и суммируется их объем. По истечении 24-х часов из _**B **_вычитается сумма исходящих за 24 часа транзакций токена WCT \(если были\). В результате получается эффективный баланс_**B. **Если его значение &gt; 0, применяем формулу \(1\) для расчета итогового веса.
