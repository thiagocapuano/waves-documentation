# Валидация транзакции с помощью функции sigVerify, без использования скрипта аккаунта

Перед тем как попасть в блок, каждая транзакция в блокчейне Waves проходит проверку на валидность. В блокчейн Waves вшита функция sigVerify, которая выполняет проверку каждой транзакции на валидность:
...
После того, как к аккаунту привязывается скрипт, все отправляемые с данного аккаунта транзакции проверяются уже этим скриптом, а не функцией sigVerify. Тем не менее, внутри скрипта аккаунта разработчики часто используют функцию sigVerify.
...
1. Объявление вызываемых функций
