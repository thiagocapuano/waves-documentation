...
**Транзакция сжигания токенов** — [транзакция](/blockchain/transaction.md), которая позволяет отдавать [токены](/blockchain/token.md) WAVES в лизинг.
...
| Название поля | Название JSON-поля | Описание |
| :--- | :--- | :--- |
| ID токена | feeAssetId | В настоящее время поле не используется. <br>Значение равно null |
